                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-sm text-left text-gray-500">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                                    <tr>
                                        <th className="px-4 py-2">Nama Sensus/Survei</th>
                                    ...
                                    {currentMonthTasks.length === 0 ? (
                                        ...
                                        currentMonthTasks.map((task, idx) => (
                                            <tr key={idx} className="bg-white hover:bg-gray-50 transition">
                                                <td className="px-4 py-2 font-medium text-gray-900">{task.nama_kegiatan}</td>
                                                <td className="px-4 py-2">{task.nama_sub_kegiatan}</td>
                        ...
                        {sortedYears.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {sortedYears.map(year => {
                                    const monthsInYear = Object.keys(groupedHistory[year]).sort((a, b) => a - b);
                                    ...
                                    return (
                                        <div key={year} className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                                            <button 
                                                ...
                                                className={`w-full flex items-center justify-between px-5 py-4 transition-colors ${isYearExpanded ? 'bg-blue-50/80 border-b border-blue-100' : 'bg-gray-50 hover:bg-gray-100'}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <span className={`text-xl font-extrabold ${isYearExpanded ? 'text-[#1A2A80]' : 'text-gray-600'}`}>{year}</span>
                                                </div>
                                                <div className={`transition-transform duration-300 ${isYearExpanded ? 'rotate-180 text-[#1A2A80]' : 'text-gray-400'}`}>
                                                    <FaChevronDown />
                                                </div>
                                            </button>
