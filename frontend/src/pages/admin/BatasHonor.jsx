...
const BatasHonor = () => {
  ...
  const currentData = aturanList.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(aturanList.length / itemsPerPage);
  ...
  return (
    <div className="max-w-6xl mx-auto py-8">
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
        ...
        {/* --- PAGINATION FOOTER --- */}
        {aturanList.length > itemsPerPage && (
            <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50">
               <div className="text-xs text-gray-500">
                  Menampilkan {indexOfFirstItem + 1} - {Math.min(indexOfLastItem, aturanList.length)} dari <strong>{aturanList.length}</strong> data
               </div>
