...
const ManageKegiatan = () => {
  ...
  const getComputedStatus = (startDate, endDate) => {
    if (!startDate) return { label: '?', className: 'bg-gray-100' };

    const now = new Date();
    const start = new Date(startDate);

    // Set waktu ke nol untuk perbandingan tanggal yang akurat
    now.setHours(0, 0, 0, 0);
    start.setHours(0, 0, 0, 0);

    // LOGIKA BARU: Jika tanggal_selesai null, gunakan tanggal_mulai sebagai acuan
    if (!endDate) {
    ...
    // LOGIKA LAMA: Jika tanggal_selesai ada
  ...
  const formatDateRange = (startDate, endDate) => {
    // Jika keduanya null atau kosong
    if (!startDate && !endDate) return '-';

    const options = { day: 'numeric', month: 'short', year: 'numeric' };

    // Format tanggal mulai jika ada
    const start = startDate
      ? new Date(startDate).toLocaleDateString('id-ID', options)
      : null;

    // Format tanggal selesai jika ada
    const end = endDate
      ? new Date(endDate).toLocaleDateString('id-ID', options)
      : null;

    // Kondisi: Jika tanggal selesai null, tampilkan hanya tanggal mulai
    if (start && !end) return start;

    // Kondisi: Jika tanggal mulai null, tampilkan hanya tanggal selesai
    if (!start && end) return end;

    // Kondisi: Jika keduanya ada
    return `${start} - ${end}`;
  };
