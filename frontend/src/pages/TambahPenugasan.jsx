...
const TambahPenugasan = () => {
  ...
  // 2. FILTER KEGIATAN DROPDOWN
  useEffect(() => {
    if (!selectedKegiatanId) {
        setListSubKegiatan([]);
        return;
    }
    // Filter Sub Kegiatan berdasarkan Parent ID
    const subs = allSubKegiatan.filter(sub => String(sub.id_kegiatan) === String(selectedKegiatanId));
    setListSubKegiatan(subs);
  }, [selectedKegiatanId, allSubKegiatan]);
  ...
  // 5. FILTER MITRA SUDAH BERTUGAS DI KEGIATAN INI
  ...
  const handleNextStep = () => {
    if (!selectedKegiatanId || !selectedSubId) {
      Swal.fire('Perhatian', 'Silakan pilih Survei/Sensus dan Kegiatan terlebih dahulu.', 'warning');
      return;
    }
    setStep(2);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Cek apakah sudah ada "Wadah Penugasan" untuk sub kegiatan ini
      const existingPenugasan = listPenugasan.find(
        p => String(p.id_subkegiatan) === String(selectedSubId)
      );

      if (existingPenugasan) {
        // Jika sudah ada, tambahkan mitra ke penugasan tersebut
        ...
        Swal.fire({
            title: 'Berhasil',
            text: `Mitra berhasil ditambahkan ke tim kegiatan ini.`,
            icon: 'success',
            timer: 2000,
        ...
        // Jika belum ada, BUAT penugasan baru (sebagai user/admin)
        const payload = {
            id_subkegiatan: selectedSubId,
            id_pengawas: user ? user.id : 1, // User yang login jadi pengawas
        ...
        Swal.fire({
            title: 'Berhasil',
            text: 'Tim baru berhasil dibuat dan mitra telah ditambahkan.',
            icon: 'success',
            timer: 2000,
            showConfirmButton: false
        }).then(() => navigate('/penugasan'));
  ...
  const filteredMitra = listMitra.filter(m => {
    ...
    // 2. Filter Sudah Terpilih di sesi ini
    const notSelected = !selectedMitras.some(selected => selected.id === m.id);
    
    // 3. Filter Sudah Ada di Database untuk Kegiatan ini
  ...
  return (
    // Jarak pt-24 untuk navigasi
    <div className={`mx-auto pb-20 pt-24 px-4 transition-all duration-300 ${step === 1 ? 'max-w-2xl' : 'max-w-6xl'}`}>
      
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-800">Plotting Tim Kegiatan</h1>
        <div className="flex items-center gap-2 text-sm text-gray-500 mt-2">
          ...
          <span className={`px-3 py-1 rounded-full ${step === 2 ? 'bg-[#1A2A80] text-white font-bold' : 'bg-gray-200'}`}>2. Pilih Mitra & Alokasi</span>
        </div>
      </div>
        ...
        {step === 1 && (
          <div className="p-8 animate-fade-in-up flex-1 flex flex-col">
            <h2 className="text-lg font-bold text-gray-700 mb-6 flex items-center gap-2">
              <FaClipboardList className="text-[#1A2A80]" /> Tentukan Sasaran
            </h2>

            <div className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-gray-600 mb-2">Pilih Survei/Sensus Utama</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-bold text-gray-600 mb-2">Pilih Kegiatan</label>
                <select 
          ...
          <div className="p-8 animate-fade-in-up flex-1 flex flex-col">
            ...
            <div className="mb-8 p-5 bg-gray-50 border border-gray-200 rounded-xl">
               ...
               {availableJabatan.length === 0 ? (
                 <p className="text-sm text-red-500 italic">Belum ada data honorarium (Jabatan) untuk kegiatan ini.</p>
               ) : (
