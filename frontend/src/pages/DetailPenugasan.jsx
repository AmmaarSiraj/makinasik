...
import { 
  ...
  FaBoxOpen, FaChartBar, FaTasks, FaEdit, FaExclamationCircle, FaCheckCircle
} from 'react-icons/fa';
...
const DetailPenugasan = () => {
  ...
  const handleUpdateMember = async (e) => {
    ...
    try {
        ...
        Swal.fire({
            ...
            text: 'Data tugas anggota berhasil diperbarui.', timer: 1500, showConfirmButton: false, toast: true, position: 'top-end'
        });
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-gray-500 font-medium">Memuat detail penugasan...</p>
    </div>
  );

  if (!penugasan) return <div className="text-center py-20 text-red-500 font-bold">Data penugasan tidak ditemukan.</div>;
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-2 ml-1">
                        Survei Induk: <span className="font-bold text-gray-700">{penugasan.nama_kegiatan}</span>
                    </p>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-bold text-gray-900 truncate max-w-[200px]" title={penugasan.nama_pengawas}>{penugasan.nama_pengawas}</p>
                        <p className="text-xs text-gray-500 truncate max-w-[200px]">{penugasan.email_pengawas}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleUpdateMember} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                        ...
                        <button 
                            ...
                            className={`px-6 py-2.5 bg-[#1A2A80] text-white text-sm font-bold rounded-xl hover:bg-blue-900 transition shadow-lg hover:shadow-xl ${calculateFinalStats.isOver ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            Simpan Perubahan
                        </button>
                    </div>
